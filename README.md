...
| **skill-autoresearch: packet-first structural hardening** | Tightened `skill-autoresearch` into a smaller repo-local ratcheting front door. It now chooses one packet (`benchmark-readiness`, `charter-freeze`, `baseline-score`, `one-change-mutation`, `support-sync`, `final-report`, or `route-out`), adds `references/run-packets-and-route-outs.md`, expands `evals/evals.json` with hosted-platform and self-application cases, and syncs compact/manifest/setup wording so the repo advertises `skill-autoresearch` as a markdown/git ratchet rather than a generic eval platform. |
...
| `skill-autoresearch` | Repo-local skill ratcheting loop: choose one packet (readiness, charter, baseline, mutation, support-sync, final report), freeze evals, keep or revert by score, and route hosted eval / ML autoresearch work outward | All |
...
| `opencontext` | Routing-first active project/repo memory — choose memory-layer choice, load-context, search-context, store-conclusions, setup-integration, or repo-packer route-out for manifests, stable links, and cross-agent handoff packets | All |

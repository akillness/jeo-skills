...
| `omx` | `omx`, `$plan`, `$ralph`, `$team`, `$ulw`, `$ultraqa`, `$deep-interview`, `$ralplan` | Codex | Codex workflow layer with Claude parity — `$team` for coordinated workers, `$ulw`/`$ultrawork` for burst parallelism, `$ultraqa` for QA fan-out, plus tmux/psmux team runtime and omx explore/sparkshell |
| `ohmg` | `ohmg`, `oh-my-agent`, `oma`, `.agents`, `/orchestrate`, `/review` | Gemini / Antigravity | Portable OMA harness entry — keeps `.agents` canonical, regenerates vendor views with `oma link`, and maps team/ultrawork/ultraqa intents to `/orchestrate`, `/ultrawork`, `/review`, or `oma agent:parallel` with Antigravity limits stated |

...
| **performance-optimization: 병목 워크플로우 재작성** | `performance-optimization`을 일반적인 React/DB 최적화 덤프에서 측정 우선 튜닝 앵커로 재구성했습니다. 이제 interaction, page-load, API, DB, capacity, memory, runtime bottleneck 모드를 분류하고 적절한 evidence source를 먼저 고른 뒤 수정안을 제안합니다. 또한 `monitoring-observability`, `debugging`, `code-refactoring`, `testing-strategies`, `game-performance-profiler`로의 route-out과 `references/tuning-modes.md`, `references/handoff-boundaries.md`, `references/measurement-checklist.md`, `evals/evals.json`을 추가했습니다. |

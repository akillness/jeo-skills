...
**카테고리형 스킬 210개 · 경량 선택 설치 · 압축 TOON 카탈로그 · 멀티플랫폼**
...
210개 스킬 폴더 전체가 아니라 **`jeo-skill` 라우터부터 설치**합니다. 카테고리,
...
> 중앙 매니페스트: `.agent-skills/skills.json` · 210개 스킬 · 10개 기본 카테고리 · 하위 카테고리/인터페이스/관계 그룹 지원
...
하위 분류: `image` (5), `video` (10), `motion` (1), `audio` (1), `presentation` (1), `diagram` (1), `design` (1), `capture` (0), `storytelling` (1)
...
```text
├── .agent-skills/          ← 210개 스킬 폴더 (SKILL.md + 선택적 지원 파일)

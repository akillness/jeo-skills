...
| 변경 | 상세 |
|------|------|
| **ooo: cli-anything harness 실행을 run 단계에 연동** | `ooo`의 execute 단계를 설치 시점에 **cli-anything**과 연동해 셋업 직후 바로 사용 가능하게 했습니다: `scripts/install.sh`에 `OOO_CLI_ANYTHING=1`(기본; `CLI_ANYTHING_HUB_SPEC`으로 핀 고정) knob이 추가되어 sibling `cli-anything` installer(uv-tool/pip fallback)로 CLI-Hub 패키지 매니저를 설치합니다. run 단계는 GUI 자동화나 맨손 셸 추측 대신 agent-native harness로 실제 소프트웨어를 구동합니다 — `cli-hub search` → `install` → `launch` — 모든 harness 명령이 `--json`을 지원하고, 그 구조화된 출력이 **evaluate 단계가 인정하는 증거**입니다(exit code가 아닌 아티팩트 — magic bytes·구조·픽셀 검증). 전체 흐름은 `Interview → Seed → Plan (spec-kit) → Execute (cli-anything) → Evaluate → Evolve`; 새 운영 규칙 #8(레지스트리 우선, harness를 seed 제약에 명시해 tool drift = spec drift). `SKILL.md`(3번째 바인딩 + execute 규칙), `SKILL.toon`, `evals/evals.json`(+2 케이스), 카탈로그 표면(`README.md`, `README.ko.md`, `setup-all-skills-prompt.md` Step 3c + Rule 1, `skills.json`/`skills.toon`, `skills-lock.json` `cli-anything` recommended extension)을 갱신했습니다. 146개 스킬(수 변화 없음). |

...
| 스킬 | 키워드 | 플랫폼 | 설명 |
|------|--------|--------|------|
| `ooo` | `ooo`, `ouroboros`, `ooo interview` | 전체 | 스펙 우선 제어 루프 — git 데이터 근거 인터뷰, 계약 동결, spec-kit 실행 계획, 실행, 검증. 모호하거나 다단계 작업의 진입점 |
...
| `ooo` | `ooo`, `ouroboros`, `ooo ralph` | 전체 | Ouroboros 스펙 우선 개발 루프 — 라이브 git 데이터에 근거한 소크라테스식 인터뷰, 불변 seed/spec, spec-kit 실행 계획(seed → `/speckit.plan` → `/speckit.tasks`), 드리프트 인식 실행, 검증 통과까지 이어가는 완료 루프. 플러그인: `claude plugin marketplace add Q00/ouroboros` |
...
| `scrapling` | 한 개의 intake packet으로 parser-only, HTTP fetch, JS 브라우저, stealth escalation, MCP, spiders 중 가장 가벼운 경로를 고르는 라우팅 우선 적응형 웹 스크래핑. 플러그인: `npx skills add https://github.com/akillness/jeo-skills --skill scrapling` | 전체 |
...
스펙 우선 개발 프런트도어입니다. **git 데이터에 근거한 인터뷰**로 모호한 요청을 명확히 하고, 계약을 동결하고, **spec-kit으로 실행 계획을 렌더링**한 뒤 실행하고, 완료 전에 검증합니다. MCP 서버 설치: `claude mcp add ooo -s user -- ouroboros mcp`.
...
**업데이트되는 git 데이터에 근거한** 소크라테스식 인터뷰 → 불변 seed/spec 고정 → **spec-kit이 seed로부터 실행 계획을 렌더링** → 그 계약을 기준으로 실행 → 완료 주장 전에 검증 → 실제로 검증될 때까지 반복합니다. Claude Code 플러그인 또는 pip으로 설치 가능하며, 스킬 installer가 두 연동을 기본으로 배선합니다.
...
# 원샷 installer: 스킬 + ouroboros-ai + git-aware 인터뷰 + spec-kit
bash .agent-skills/ooo/scripts/install.sh
# knob: OOO_GIT_INTERVIEW=0 · OOO_SPEC_KIT=0 · SPEC_KIT_REF=<ref>
...
| 변경 | 상세 |
|------|------|
| **ooo: git 데이터 근거 인터뷰 + spec-kit 실행 계획** | `ooo`(Ouroboros) 스킬의 인터뷰 철학을 **업데이트되는 git 데이터**에, 실행 계획 단계를 **spec-kit**에 연동했습니다. 새 `scripts/git-interview-context.sh`가 라이브 git 근거(브랜치/HEAD, 최근 커밋, churn 핫스팟, 활동 기여자, working-tree 상태)로 `.ouroboros/interview-context.md`를 재생성해, 소크라테스식 인터뷰의 brownfield **Context 가중치(15%)**를 *지금* 리포지토리 상태 기준으로 채점합니다 — churn 핫스팟은 인터뷰 질문이 되고, dirty working tree는 가정이 아니라 질문입니다. 새 `scripts/install.sh`가 설치 시점에 두 연동을 지정합니다(`OOO_GIT_INTERVIEW=1`, `OOO_SPEC_KIT=1` 기본; `SPEC_KIT_REF`로 핀 고정): seed 동결 후 spec-kit이 `/speckit.plan` → `/speckit.tasks`로 **seed로부터** 검토 가능한 실행 계획을 렌더링합니다(seed → plan 단방향; seed가 계약 SSOT — spec 우선 작성은 여전히 `spec-stack`). 전체 흐름은 `git data → Interview → Seed → Plan (spec-kit) → Execute → Evaluate → Evolve`. `SKILL.md`(운영 규칙 +2), `SKILL.toon`, 신규 `evals/evals.json`, 카탈로그 표면(`README.md`, `README.ko.md`, `setup-all-skills-prompt.md` Step 3c + Rule 1, `skills.json`/`skills.toon`, `skills-lock.json`)을 갱신했습니다. 146개 스킬(수 변화 없음). |

...
**127개 로컬 스킬 폴더 · 설치 가능 스킬 128개 · TOON 포맷 · 멀티플랫폼**
...
Agent Skills는 LLM 기반 개발 워크플로우를 위한 컬렉션으로, 현재 127개 로컬 스킬 폴더와 127개 설치 가능 스킬을 제공합니다. `ooo`(스펙 우선), `bmad`(계획), `plannotator`(검토)를 핵심 오케스트레이션 트리오로 구축되었으며 다음을 제공합니다:
...
| Antigravity CLI | `~/.gemini/antigravity/hooks/toon-skill-inject.sh` | `includeDirectories` 세션 로드 |

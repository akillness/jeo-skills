...
**카테고리형 스킬 213개 · 경량 선택 설치 · 압축 TOON 카탈로그 · 멀티플랫폼**
...
213개 스킬 폴더 전체가 아니라 **`jeo-skill` 라우터부터 설치**합니다. 카테고리,
...
> 중앙 매니페스트: `.agent-skills/skills.json` · 213개 스킬 · 10개 기본 카테고리 · 하위 카테고리/인터페이스/관계 그룹 지원
...
하위 분류: `client` (2), `web` (2), `server` (0), `design-ui` (4), `audio` (1), `animation` (2), `motion-vfx` (2), `sprite-image` (1), `art-resources` (0), `storytelling` (0), `tooling` (3), `qa-performance` (4), `release` (2)
...
```text
├── .agent-skills/          ← 213개 스킬 폴더 (SKILL.md + 선택적 지원 파일)

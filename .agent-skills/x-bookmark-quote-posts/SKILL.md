...
description: >
  Check a user's latest X/Twitter bookmarks and turn recent saved posts into source-backed
  quote-post drafts calibrated against the user's latest 100 authored posts. Use when asked to
  review X bookmarks, create quote posts from bookmarks, refresh a bookmark quote queue, run a
  bookmark quote automation, study a user's X voice, or write first-person quote posts from X
  sources.
...
license: MIT
metadata:
  tags: bookmark, quote, posts, mengto-skills, codex-workflow
  version: "1.0"
...
This skill is voice-specific: it calibrates drafts to one particular X account's established, first-person voice rather than producing generic commentary. Use it when:

- Reviewing a user's latest X bookmarks and turning them into source-backed, dated quote-post drafts.
- Calibrating the draft voice against that specific account's latest 100 authored posts (originals, replies, quote posts) before drafting anything.
- Refreshing or maintaining a dated bookmark quote-post queue file, such as `data/x-growth/bookmark-quote-posts/YYYY-MM-DD.md`.
- Producing a ranked set of 7-10 quote-post drafts (Best Picks / Secondary Picks) plus a suggested-tweet summary table for the user to react to.
- Working through the Codex in-app browser against X specifically, under strict no-posting/no-reply/no-mutation rules.
- Not for general X/Twitter data collection — searching tweets, user lookup, follower extraction, media download, monitoring, or posting/liking/DMing without voice-matched drafting. For raw X data access, use this repo's `x-twitter-scraper` skill instead.
...
- [Upstream source: MengTo/Skills — x-bookmark-quote-posts](https://github.com/MengTo/Skills/tree/main/agent-skills/codex/x-bookmark-quote-posts)
- [`agents/openai.yaml`](agents/openai.yaml) — OpenAI-surface display name, short description, and default prompt for this skill.

...
description: >
  Create a recurring daily UI inspiration capture. Use when the user asks to run, refresh,
  package, or validate dated UI inspiration bundles, especially for
  `articles/YYYY-MM-DD-ui-inspiration-capture/` outputs, Framer/Dribbble landing-page
  inspiration, motion-study screenshots/videos, AI-builder prompts, duplicate checking, or
  converting a project runbook into repeatable workflow.
...
license: MIT
metadata:
  tags: daily, inspiration, capture, mengto-skills, codex-workflow
  version: "1.0"
...
- The user asks to run, refresh, or package the recurring daily UI inspiration capture into `articles/YYYY-MM-DD-ui-inspiration-capture/`
- The task involves collecting exactly 5 live landing-page/product references (Framer Marketplace, Dribbble as best-effort) with full-page scroll screenshots, section-by-section crops, and motion-frame images
- The task requires building a `manifest.json` + `content.md` article bundle with AI-builder prompts, not a plain screenshot dump or README
- The user wants duplicate-checking against previous `articles/*-ui-inspiration-capture/manifest.json` runs before committing
- The task is verifying an existing capture bundle against the output contract (item count, full-page/section evidence, motion frames, no `README.md`)
- Not for: ad-hoc one-off screenshots, unrelated content/article work, or generating a `README.md` — those fall outside this workflow's contract and should follow the project's own conventions instead
...
- Upstream source: [MengTo/Skills — daily-ui-inspiration-capture](https://github.com/MengTo/Skills/tree/main/agent-skills/codex/daily-ui-inspiration-capture)
- [agents/openai.yaml](agents/openai.yaml) — bundled agent interface metadata

...
description: >
  Write, rewrite, review, or continuously refine X/Twitter posts in Meng To's current voice
  using his deduplicated authored-post corpus, personal and product context, shared resources,
  and Content repo evidence. Use when asked to improve a tweet in Meng's tone, draft an original
  post, reply, thread, resource share, or quote post as Meng, check whether an angle repeats an
  earlier post, study Meng's writing style, ingest another 20-50 authored posts, or hone the
  reusable Meng X voice profile.
...
license: MIT
metadata:
  tags: write, meng, mengto-skills, codex-workflow
  version: "1.0"
...
This skill writes in **Meng To's specific, personal voice** — it is not a generic ghostwriting or copywriting skill, and its remix patterns and voice profile should not be reused for any other author or brand. Use it when:

- Rewriting or polishing a tweet, reply, thread, quote post, resource share, or product post so it reads like Meng To wrote it, drawing on his deduplicated authored-post corpus and Content repo evidence.
- Drafting an original X post as Meng, grounded only in verified products, workflows, resources, or personal details already recorded in the Content repo.
- Checking whether a new angle repeats an earlier authored post, hook, or recurring belief (corpus and editorial duplication checks).
- Ingesting a fresh batch of 20-50 unseen authored X/Twitter posts to keep `references/tweet-corpus.jsonl` current.
- Honing or updating the reusable `references/voice-profile.md` after a new batch confirms a pattern, adds lived context, or reveals a format-specific move.
- Not for generic ghostwriting in someone else's voice, and not for collecting raw X/Twitter data (search, scraping, monitoring, media download) — for that, use this repo's `x-twitter-scraper` skill (read its SKILL.md first) and feed the collected posts into this skill's ingestion script.
...
- Upstream source: https://github.com/MengTo/Skills/tree/main/agent-skills/codex/write-like-meng-on-x
- [references/voice-profile.md](references/voice-profile.md)
- [references/content-source-map.md](references/content-source-map.md)
- [references/tweet-corpus.jsonl](references/tweet-corpus.jsonl)
- [scripts/update-tweet-corpus.mjs](scripts/update-tweet-corpus.mjs)

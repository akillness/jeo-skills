...
- **Run-slug integrity guard (mandatory):** persist one slug source-of-truth (for example `.survey/LATEST_SLUG`) and pass it explicitly to every generation step. Do not rediscover active run folders via `find .survey ... | head -1` style heuristics.

...
- **GitHub search license-shape normalization:** when using `gh search repos ... --json license`, treat `license.key`/`license.name` (and `spdxId` when present) as valid license evidence before marking unknown-license. Only trigger REST enrichment/fallback after this normalization step to avoid false degraded `license` causes.
...
- If a lane remains `raw_count == 0` after documented recovery attempts, set `lane_status: degraded` and include `no-results` in `degraded_causes` explicitly (do not leave causes empty).
- If **all five mandatory lanes** end with `raw_count == 0` in the same run, set a run-level `transport_status.sweep_health = "degraded-all-lanes-zero-results"`, persist the triggering command/error artifact paths, and treat the run as retrieval-degraded (do not present candidate quality conclusions as ecosystem absence).
...
- Apply a lane-metric integrity check before finalizing artifacts: enforce `raw_count >= kept_count`, and when `raw_count == 0` force `zero_star_raw = 0` and `median_stars_raw = 0` to avoid impossible metric states in unattended runs.
...
- Persist provider error envelope fields when available (for example HTTP status + `tag` + `requestId` from web-search failures) as `transport_status.web_search.http_status`, `transport_status.web_search.error_tag`, and `transport_status.web_search.request_ids[]` so outages remain traceable across runs.
...
- In merge carry-forward mode, if the selected PR reports `mergeStateStatus: UNKNOWN`, do not treat it as a final state on the first sample; perform bounded mergeability re-checks (short polling), and if it remains unknown, skip to the next deterministic candidate while recording the defer reason in artifacts.

...
- Persist provider error envelope fields when available (for example `tag` and `requestId` from web-search failures) as `transport_status.web_search.error_tag` and `transport_status.web_search.request_ids[]` so outages remain traceable across runs.

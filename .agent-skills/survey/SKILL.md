...
- If a lane remains `raw_count == 0` after documented recovery, always include `no-results` in `degraded_causes` so empty-lane failures are explicit and auditable.

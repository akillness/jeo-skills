...
- If a lane remains `raw_count == 0` after documented recovery (including stage-2 when configured), explicitly include `no-results` in `degraded_causes` rather than leaving cause lists empty.

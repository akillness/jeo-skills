...
description: >
  Generate multiple original brand campaign worlds from a supplied visual reference while
  controlling how close the new work feels without copying protected signature elements. Use
  when a user provides a brand identity image, poster, editorial campaign, moodboard, or
  generated concept and asks for inspired alternatives, several distinct brands in one visual
  family, a closer-to-reference V2, exact wordmarks inside images, or originality-safe
...
license: MIT
metadata:
  tags: generate, reference, inspired, brand, worlds, mengto-skills, codex-workflow
  version: "1.0"
  source: https://github.com/MengTo/Skills/tree/main/agent-skills/codex/generate-reference-inspired-brand-worlds
---
...
- A user supplies a brand identity image, poster, editorial campaign, moodboard, or generated concept and asks for inspired alternatives at a chosen similarity dial (30/50/70/85%).
- A user wants several distinct brands generated as one coherent visual-art-direction family (the distinct-brand matrix).
- A user asks for a closer-to-reference V2 revision of an existing concept while keeping the name, meaning, and core subject.
- A prompt needs an exact wordmark or brand name rendered correctly, exactly once, inside a generated image.
- A user wants originality-safe image-generation prompts that avoid reproducing protected signature elements (exact poses, motifs, layouts, logos, letterforms).
- Not for exact reproduction or pixel-for-pixel copying of a protected brand's identity, or for general image generation unrelated to brand-world creation — route those requests elsewhere.
...
- Upstream source: [MengTo/Skills — generate-reference-inspired-brand-worlds](https://github.com/MengTo/Skills/tree/main/agent-skills/codex/generate-reference-inspired-brand-worlds)
- [references/example-prompts.md](references/example-prompts.md) — MOSSWEFT, KINTRA, NIGHTJAR, TIDETURN, and EMBERHOUSE prompt patterns.
- [agents/openai.yaml](agents/openai.yaml) — agent configuration for this skill.

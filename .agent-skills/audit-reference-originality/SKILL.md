...
description: >
  Audit a website or digital experience against its supplied source references for originality
  and plagiarism risk. Use when Codex must compare current or historical site output with
  reference pages, capture packs, screenshots, copy, brands, numbers, images, assets, videos,
  layouts, motion, or code; raise evidence-backed red flags; distinguish common visual grammar
  from distinctive copying; and propose concrete fixes without making unsupported legal claims.
...
license: MIT
metadata:
  tags: audit, reference, originality, mengto-skills, codex-workflow
  version: "1.0"
...
- When a supplied reference corpus exists (manifests, prompt packs, capture packs, stills, MP4s, reference URLs, `IMAGE_CREDITS.md`) and a shipped site or experience must be checked against it for originality/plagiarism risk.
- When you need to compare current or historical site output with references across text, brands, numbers, images, assets, videos, layout, motion, or code.
- When red flags must be evidence-backed — each paired with exact current-site and reference-site locations, severity, and a proposed fix — rather than a subjective "vibe check".
- When you need to distinguish common visual grammar (black backgrounds, large sans-serif type, standard fade-ins) from distinctive, provably-copied elements.
- When copied material may have been renamed, recolored, cropped, or deleted, requiring `git log`/`git show`/`git log -S` history checks in addition to current files.
- Not for making the actual replacement edits unless the user also asks for fixes (this skill audits and proposes fixes; it does not silently apply them), and not for general code review or security audits unrelated to originality — route those to standard review/fix workflows instead.
...
- Upstream source: [MengTo/Skills — audit-reference-originality](https://github.com/MengTo/Skills/tree/main/agent-skills/codex/audit-reference-originality)
- [references/audit-rubric.md](references/audit-rubric.md)
- [scripts/build_evidence_inventory.py](scripts/build_evidence_inventory.py)

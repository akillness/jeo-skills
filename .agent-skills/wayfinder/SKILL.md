...
description: >
  ...
  tickets on your issue tracker, and resolve them one at a time until the way to the destination
  is clear. Use when the work is too large for one agent session and needs a shared decision map
  first. Triggers on: wayfinder, map this out, too big to plan in one go.
allowed-tools: Read Grep Glob Bash Write Edit
compatibility: >
  For work larger than one agent session can hold. Route executable slicing to to-tickets and
  milestone shaping to task-planning.
metadata:
  tags: decision-tickets, wayfinding-map, multi-session-planning, issue-tracker, big-chunk
...
Plan a huge chunk of work (more than one agent session can hold) as a shared map of decision tickets on your issue tracker, and resolve them one at a time until the way to the destination is clear. Use when the work is too large for one agent session and needs a shared decision map first. Triggers on: wayfinder, map this out, too big to plan in one go.
...
- Use when the work is too large for one agent session and needs a shared decision map first.
- Triggers on: wayfinder, map this out, too big to plan in one go.
...
- Upstream skill: `mattpocock/skills` `skills/engineering/wayfinder/SKILL.md` (commit `3cca18b`, MIT)

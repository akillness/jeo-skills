...
- **`qa`**: P4 independent verification discipline; this harness adds its mandatory adversarial 20-case contract.
...
2. Give every artifact a `run_id`, owner, creation time, input artifact versions/hashes, status, and decision IDs. Valid statuses are `draft`, `evidence-ready`, `blocked`, `PASS`, `FAIL`, `INCONCLUSIVE`, `ready`, `STOP-SHIP`, and `MILESTONE_CLOSED`; published immutability is separate from status.
...
| Adversarial QA lead | 20 cases, severity, regressions, PASS/FAIL | fixes, waivers, schedules, gate relaxation |
...
The engineering lead consumes immutable rules and interactions. Publish architecture, deterministic replay, performance evidence, and risky-technology validation. Require same input, seed, and version to produce the same result and traceable state transitions. Measure before optimizing and record workload, environment, unit-bearing budget, result, and replay procedure for frame time, allocations/memory, input latency, pathing, and peak combat. Keep engine-specific APIs behind future adapters; never change rule values for implementation convenience. After performance evidence is published, live operations publishes an immutable `operations-draft` that P4 may attack. Non-determinism, input loss, pathing deadlock, memory-budget breach, unvalidated risky technology, or an absent operations draft blocks P4.

### P4 — adversarial QA

The QA lead must use `qa` practice and independently execute all 20 rows in `quality-gates.md`, covering A1–A7 player archetypes. Each case records archetype, attack family, seed/input, expected and actual result, evidence path, severity, reproducibility, finite watchdog/budget with a unit, observed consumption, terminal outcome, and PASS/FAIL. Missing or non-reproducible evidence is `INCONCLUSIVE`, which is release-equivalent to `FAIL`.
...
Re-entry opens only after QA `FAIL` or operations `STOP-SHIP`, for at most two monotonic cycles:

1. QA or operations submits the defect ID, evidence, and impact scope.
...
4. Restart at P3: regenerate engineering evidence and the operations draft against the replacement artifact, then verify every immutable P3→P4 and P3→P5 handoff.
5. QA reruns affected cases plus the required regression matrix.
6. Operations reassesses readiness.

After the second cycle, a remaining `FAIL` or `STOP-SHIP` causes the director to publish `production/milestone-closure.md` with status `MILESTONE_CLOSED`. A later run requires external authorization, a new run ID, and a materially changed intake/constraints fingerprint. Never retry the same stimulus implicitly, create a successor run automatically, erase evidence, or use re-entry as a veto override.
...
- “Validate our unlimited-growth economy, direct-price catalog, deterministic replay, and twenty adversarial QA cases before release.”

---
name: excalidraw-diagram-master
description: Create Excalidraw diagram JSON files that make visual arguments. Use when the user wants to visualize workflows, architectures, or concepts with hand-drawn or modern aesthetics.
compatibility: opencode
---

# Excalidraw Diagram Master Skill

Generate `.excalidraw` JSON files that **argue visually**, not just display information. Combine deep technical research with clear design patterns and precise JSON implementation.

## Harness Voice

- Follow `STYLE_GUIDE.md` and keep diagram guidance concrete, testable, and non-generic.

**Setup:** Files can be opened at [excalidraw.com](https://excalidraw.com) or uploaded for shareable links using the upload script.
...
**For comprehensive or technical diagrams, you MUST build the JSON one section at a time.** Do NOT attempt to generate the entire file in a single pass. This is a hard constraint — output limits and quality concerns make section-by-section the only viable approach.
...
- **Don't generate the entire diagram in one response.** You will produce truncated, broken JSON. Even if the diagram is small enough to fit, splitting into sections produces better results.
- **Don't use a coding agent** to generate the JSON. The agent won't have sufficient context about the skill's rules.
- **Don't write a Python generator script.** The templating introduces indirection that makes debugging harder. Hand-crafted JSON with descriptive IDs is more maintainable.
...
Colors encode information, not decoration. Use semantic color choices:

| Use Case | Fill Color | Stroke Color |
|----------|------------|--------------|
| Primary / Input | `#a5d8ff` (Blue) | `#1971c2` |
| Success / Output | `#b2f2bb` (Green) | `#2f9e44` |
| Warning / External | `#ffd8a8` (Orange) | `#e8590c` |
| Processing / Special | `#d0bfff` (Purple) | `#7950f2` |
| Error / Critical | `#ffc9c9` (Red) | `#e03131` |
| Notes / Decisions | `#fff3bf` (Yellow) | `#fab005` |
| Storage / Data | `#c3fae8` (Teal) | `#12b886` |
...
- Evidence artifacts use dark backgrounds with colored text
...
## JSON Structure & Element Templates
...
### Default Styles (Can Be Omitted)
- `strokeColor`: `"#1e1e1e"`
- `backgroundColor`: `"transparent"`
- `fillStyle`: `"solid"`
- `strokeWidth`: `2`
- `roughness`: `1` (use `0` for modern/clean)
- `opacity`: `100`
...
**CRITICAL**: Do NOT use `"label": { "text": "..." }` on shapes. This is NOT a valid Excalidraw property and will be silently ignored, producing blank shapes. You MUST use the container binding approach below.
...
- Use `fontFamily: 1` (Virgil/hand-drawn) or `3` (Cascadia/modern)
...
### Labeled Arrow
Same container binding approach:
```json
{
  "type": "arrow",
  "id": "a1",
...
### Standalone Text (Free-Floating)
Use for titles and annotations only (not labels on shapes):
```json
{
  "type": "text",
  "id": "t1",
...
## Uploading for Shareable Links

After creating a `.excalidraw` file, you can upload it to excalidraw.com (no account needed) for a shareable URL:

```bash
python scripts/upload.py ~/path/to/diagram.excalidraw
```
...
17. **Font**: `fontFamily: 3` (modern) or `1` (hand-drawn)

...
1. Confirm target hardware and whether the measurement is from editor or packaged build.
2. Lock a reproducible route: exact scene, level, encounter, traversal path, or menu.
3. Decide whether the first question is **steady low FPS** or **intermittent hitching**.
...
| `loading-streaming-io` | Traversal hitch, asset reveal stutter, scene transition stalls, streaming pop-in | Loading-related evidence, timeline around traversal, addressable/content load suspects | Unreal Insights load-time traces, traversal route, streaming-related timing | Optimizing steady-state render cost first |
| `platform-config-thermal-device-specific` | Editor numbers differ from device, issue appears only on Steam Deck/mobile/VR, or worsens over time | Packaged build/device validation, quality settings, platform toggles | Target-device capture, packaged build, platform-specific stats | Trusting editor results as final truth |

## Recommended next artifacts by bucket
- `cpu-gameplay-scripting` → CPU/GPU split note or profiling plan
- `cpu-render-thread-draw-call-pressure` → benchmark scene brief or render-cost checklist
...
- `platform-config-thermal-device-specific` → platform review brief
- `unknown-needs-better-capture` → evidence request packet

## Minimum evidence request packet
- one representative screenshot or trace summary
- target hardware and build type
- exact repro route or encounter

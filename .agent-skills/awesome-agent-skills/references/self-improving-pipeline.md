...
The ADK multi-agent skill optimizer, merged from the upstream `self-improving-agent-skills` folder. The routing guidance works everywhere; running the full app needs Python and a Gemini API key.
...
The upstream app bundles a `content-writer` example skill (the `code-reviewer` example is omitted here, in line with this consolidation):

- **content-writer**: Writes marketing copy following style guidelines

Create a zip file from the example:

```bash
cd example_skills
zip -r content-writer.zip content-writer/
```

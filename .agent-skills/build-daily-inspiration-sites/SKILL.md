...
description: >
  Turn a completed daily UI inspiration capture into exactly five original landing-page builds,
  one per separate Codex task, using Sites. Use when the user asks to turn the daily inspiration
  references, a five-item UI prompt pack, or a dated `*-ui-inspiration-capture` article into
  distinct HTML landing pages while changing the source brands, names, copy, people, numbers,
  pricing, claims, and imagery.
...
license: MIT
metadata:
  tags: build, daily, inspiration, sites, mengto-skills, codex-workflow
  version: "1.0"
...
- Use when the user asks to turn a completed daily UI inspiration capture — a dated `*-ui-inspiration-capture` article with a five-item manifest — into five original landing-page builds.
- Use when converting a five-item UI prompt pack into five independent Codex tasks, each owning one build, one commit, and one private Sites deployment.
- Use when the source items must stay art direction only, never a template to copy, and every item needs a materially distinct replacement brand, copy, and imagery identity.
- Use when preparing one build brief per manifest item with a single inline local full-page image attachment, following `references/thread-brief-contract.md`.
- Use when validating a capture folder's `manifest.json` and `content.md` with `scripts/validate_capture.mjs` before dispatching build tasks.
- Not for recollecting the original inspiration capture, editing an existing site build directly, or producing anything other than exactly five distinct sites — route capture creation to the daily UI inspiration capture workflow instead.
...
- Upstream source: [MengTo/Skills — build-daily-inspiration-sites](https://github.com/MengTo/Skills/tree/main/agent-skills/codex/build-daily-inspiration-sites)
- [references/thread-brief-contract.md](references/thread-brief-contract.md) — the build-brief template used in step 3
- [scripts/validate_capture.mjs](scripts/validate_capture.mjs) — the capture validator run in step 1
- [agents/openai.yaml](agents/openai.yaml) — bundled agent configuration

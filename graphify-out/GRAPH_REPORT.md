...
3. `obsidian-cli` is no longer just a command catalog: the skill now separates CLI command/TUI mode, developer-command mode, official `obsidian://` handoff, and route-outs for headless or plugin/API-heavy jobs.
4. The refreshed `obsidian-cli` support packet corrected installation drift against current official docs by centering installer `1.12.7+`, macOS `/usr/local/bin/obsidian`, and Linux `~/.local/bin/obsidian` behavior instead of older packaging-specific guesses.
...
- `obsidian-cli` should stay distinct from `llm-wiki`, `opencontext`, and `graphify` by owning desktop Obsidian control and URI handoff rather than narrative knowledge maintenance or repo memory.
- `obsidian-cli` should also stay distinct from `obsidian-plugin` by owning app operation and plugin-command execution, not plugin project scaffolding or submission workflows.
